...
def build_vocab(files, tag_file, output_dir, min_count=MINCOUNT):
    # 1. Words
    ...
    with Path('{}/vocab.words.txt'.format(output_dir)).open('w') as f:
    ...
    with Path('{}/vocab.chars.txt'.format(output_dir)).open('w') as f:
    ...
    vocab_tags = set()
    with Path(tag_file).open() as f:
        for line in f:
            vocab_tags.update(line.strip().split())

    with Path('{}/vocab.tags.txt'.format(output_dir)).open('w') as f:
